...
- dry-run mode
- full logging
- backs up BOTH files and directories inside the source
"""
...
def run_backup(freq, src_dir, dest_root, dry_run=False):
  ...
  # Loop over entries (files AND directories) in source
  for entry in src.iterdir():
    ...
    # Create tar.zst archive of this entry (file or directory)
    ...
    # Find previous backups for this entry
    ...
    if dry_run:
      ...
      # temp file not needed in dry-run
  ...
  if retention_days is not None:
    ...
    # Retention per entry (file or directory) name
    for entry in src.iterdir():
      name = entry.name
      ...
      # Keep newest, evaluate older ones
      for fpath_str in files[:-1]:
